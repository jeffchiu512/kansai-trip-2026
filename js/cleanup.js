...
function cleanDiscussionArtifacts() {
  ...
  // D2 — present only the final city route, not how it changed during planning.
  ...
  // D4 — remove references to discarded alternatives such as HARBS / Tsutenkaku comparisons.
  ...
  // D5 — describe the final Uji → Abeno route directly.
  ...
  // D6 — keep only actionable USJ guidance.
  setHtml('[data-event-id="d6-usj-arrival"] .spot-desc', "全天行程集中在 USJ。依官方開園時間提早 60～90 分鐘抵達園區，入園後立即確認任天堂世界與各項整理券狀態。");

  // D7 — describe the final Rinku stop without mentioning its former day.
...
const observer = new MutationObserver(() => cleanDiscussionArtifacts());
observer.observe(document.documentElement, { childList: true, subtree: true });

if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", cleanDiscussionArtifacts, { once: true });
} else {
  cleanDiscussionArtifacts();
}

setTimeout(cleanDiscussionArtifacts, 250);
setTimeout(cleanDiscussionArtifacts, 1000);
